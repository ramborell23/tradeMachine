...
function Database(cn, dc, config) {
    ...
    function extend(ctx, obj) {
         ...
         */
        obj.task = function () {
            return taskProcessor.call(this, arguments, false);
        };
         ...
         */
        obj.tx = function () {
            return taskProcessor.call(this, arguments, true);
        };
         ...
         *
         */
        obj.txIf = function (options) {
            let cnd = !obj.ctx || !obj.ctx.inTransaction;
            if (typeof options === 'object' && options !== null && 'cnd' in options) {
                cnd = typeof options.cnd === 'function' ? options.cnd.call(obj, obj) : !!options.cnd;
            }
            return taskProcessor.call(this, arguments, cnd);
        };

        // Task method;
        // Resolves with result from the callback function;
        function taskProcessor(args, isTX) {

            const options = args[0];
            const params = {
                cb: (typeof options === 'function' && options) || args[1]
            }; // {tag, mode, cb}
            ...
            taskCtx.mode = params.mode; // transaction mode;
            if (this !== obj) {
                taskCtx.context = this; // calling context object;
            }

            const tsk = new config.$npm.task.Task(taskCtx, params.tag, isTX, config);
